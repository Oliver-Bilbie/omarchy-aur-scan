...
local AGENT_CONFIG_DIRS = {
   ...
   grok = {},
   codex = {},
   -- natives are required for the omp TUI binary glue
   omp = { ".omp/natives" },
   pi = { ".pi/agent/themes" },
   crush = {},
}
...
local AGENT_STATE_DIRS = {
   ...
   codex = { ".codex", ".codex/tmp" },
   omp = { ".omp", ".omp/agent", ".omp/logs", ".omp/run" },
   pi = { ".pi", ".pi/agent" },
   crush = { ".config/crush", ".local/share/crush" },
}

-- Provider keys needed when file auth is missing. Same blast radius as copied
-- auth files if the agent still has shell (codex/crush).
...
local SHELL_AGENTS = { codex = true, crush = true }
...
local function resolve_agent()
   ...
   if not SUPPORTED_AGENTS[agent] then
      return nil, agent .. " cannot be sandboxed for AUR investigation.\n" ..
          "Choose a different default agent with: omarchy default agent <name>\n" ..
          "Supported: opencode, gemini, copilot, claude, grok, codex, omp, pi, crush\n"
   end
...
local function agent_command(agent, workspace, prompt, opts)
   ...
   elseif agent == "pi" then
      ...
      cmd[#cmd + 1] = prompt
      return cmd
   elseif agent == "crush" then
      -- No interactive --prompt flag; show the brief then open the TUI.
      local intro = "AUR scan brief is in INVESTIGATE.md (also printed above). "
          .. "Read package files in this directory and write a short verdict."
      return {
         "bash", "-lc",
         "printf '%s\\n\\n' " .. util.sh_quote(prompt)
         .. " | sed -n '1,80p'; "
         .. "printf '\\n---\\n%s\\n\\n' " .. util.sh_quote(intro)
         .. "; exec crush --cwd " .. util.sh_quote(workspace),
      }
   end
end
...
function M.launch(name, path, report)
   ...
   if SHELL_AGENTS[agent] then
      if not util.user_response("Shell cannot be disabled for " .. agent .. " in the sandbox — this increases the attack surface compared to other agents.\nContinue anyway?") then
         return
      end
   end
   ...
   elseif agent == "codex" then
      local cfg = read_file(home .. "/.codex/config.toml")
      if cfg then
         local m = cfg:match('model%s*=%s*"([^"]+)"') or cfg:match("model%s*=%s*'([^']+)'")
         if m and is_chat_model(m) then
            opts.model = m
         end
      end
   ...
   if agent == "crush" then
      local ok, werr = util.write_file(workspace .. "/INVESTIGATE.md", prompt)
      if not ok then
